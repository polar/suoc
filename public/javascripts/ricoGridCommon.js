 ...
 */
  ...
  chooseColumns: function(e) {
    ...
    if (!this.columnChooser) {
      var z=Element.getStyle(this.outerDiv.offsetParent,'z-index');
      ...
      var chooserDiv=this.columnChooser.contentDiv;
      for (var x=0;x<this.columns.length;x++) {
        var col=this.columns[x];
        var itemDiv=chooserDiv.appendChild(document.createElement('div'));
        col.ChooserBox=RicoUtil.createFormField(itemDiv,'input','checkbox');
        var span=itemDiv.appendChild(document.createElement('span'));
  ...
  exportStart: function() {
    ...
    for (var r=0; r<this.hdrCells.length; r++) {
      ...
      for (var c=0,i=0; c<this.hdrCells[r].length; c++) {
        ...
        if (newSpan > 0) {
          var divs=Element.select(hdrcell.cell,'.ricoLG_cell');
