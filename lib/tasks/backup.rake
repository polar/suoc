...
namespace :backup do
    ...
    def retrive_backup_config
      ...
      backupdir = config[RAILS_ENV] && config[RAILS_ENV][YAML_BACKUPDIR_KEY] ||
                  config[YAML_BACKUPDIR_KEY] || DEFAULT_BACKUPDIR
      archivedir = config[RAILS_ENV] && config[RAILS_ENV][YAML_ARCHIVEDIR_KEY] ||
                   config[YAML_ARCHIVEDIR_KEY] || DEFAULT_ARCHIVEDIR
      sqldumpfile = config[RAILS_ENV] && config[RAILS_ENV][YAML_SQLDUMPFILE_KEY] ||
                    config[YAML_SQLDUMPFILE_KEY] || DEFAULT_SQLDUMPFILE
      dirs        = config[RAILS_ENV] && config[RAILS_ENV][YAML_DIRS_KEY] ||
...
#       end
      ...
      if adapter == "mysql"
        ...
        cmd = "sqlite3 #{dbfile} <  #{sqldumpfile} "
      else
