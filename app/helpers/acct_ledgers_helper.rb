module AcctLedgersHelper
  ...
  def options_for_showing_action_sets(action_sets, selected_action_set_name)
    if RAILS_GEM_VERSION <= "2.2.2"
       selected = url_for(params.update( :action_set => ""))
     ...
     opts = [[ "All", url_for(params.update( :action_set => ""))]]
     for a in action_sets do
       url = url_for(params.update( :action_set => a))
       opts += [[a.name, url]]
  ...
  def options_for_showing_action_sets1(action_sets, selected_action_set_name)
    if RAILS_GEM_VERSION <= "2.2.2"
       selected = ""
       # disabled is not in RAILS 2.2.2
       disabled = "n/a"
